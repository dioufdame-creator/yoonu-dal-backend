            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              ...
              <div className="bg-emerald-50 rounded-2xl p-4 text-center">
                ...
                <p className="text-lg font-bold text-emerald-700">
                  {formatCurrencyFull(payoutAmount)}
                </p>
                <p className="text-xs text-gray-500">= {tontine?.max_participants} × contrib.</p>
              </div>
            ...
            {isFull ? (
              ...
              <div className="space-y-3">
                <button onClick={handleJoin} disabled={joining}
                  ...
                  ) : isAuthenticated ? '🦁 Rejoindre cette tontine' : '🔐 Se connecter pour rejoindre'}
                </button>

                {!isAuthenticated && (
                  <button onClick={() => { localStorage.setItem('pending_invite_code', inviteCode); onNavigate?.('register'); }}
                ...
                <p className="text-center text-xs text-gray-500">
                  {isAuthenticated ? 'Tu rejoindras immédiatement la tontine' : 'Tu rejoindras la tontine après connexion'}
                </p>
              </div>
            )}
          </div>
