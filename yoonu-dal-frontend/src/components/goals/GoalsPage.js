...
const GoalsPagePremiumV3 = ({ toast, onNavigate }) => {
  ...
  const loadGoalHistory = async (goalId) => {
    try {
      // TODO: Endpoint à créer /api/goals/{id}/history/
      // Pour l'instant, simulation
      const mockHistory = [
        { id: 1, amount: 100000, type: 'add', source: 'Manuel', note: 'Salaire Mars', created_at: '2026-03-20T10:00:00Z' },
        { id: 2, amount: 50000, type: 'add', source: 'Enveloppe Projets', note: 'Auto-allocation', created_at: '2026-03-15T08:00:00Z' },
        { id: 3, amount: 30000, type: 'remove', source: 'Manuel', note: 'Urgence médicale', created_at: '2026-03-10T14:00:00Z' },
      ];
      setGoalHistory(mockHistory);
    } catch (error) {
  ...
  const handleContribute = async (e) => {
    ...
    try {
      ...
      // TODO: Créer contribution dans historique
      // await API.post(`/goals/${selectedGoal.id}/contributions/`, {
      //   amount, type: contributeType, source: 'Manuel', note: contributeNote
      // });
          ...
          <div className="bg-white/95 rounded-2xl shadow-2xl max-w-md w-full p-6">
            ...
            <div className="space-y-3">
              ...
              <button
                ...
                className="w-full bg-gradient-to-r from-purple-600 to-pink-600 text-white px-6 py-3 rounded-xl font-semibold mt-4"
              >
